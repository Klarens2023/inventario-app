...
const TIPOS_PC        = ['Computador Desktop','Laptop','Servidor','All-in-One','Tablet']
const TIPOS_MONITOR   = ['Monitor/Pantalla']
const TIPOS_IMPRESORA = ['Impresora Láser B/N','Impresora Láser Color','Impresora Multifuncional','Impresora Inyección','Impresora Térmica','Impresora Etiquetas','Plotter']
const TIPOS_SOFTWARE  = ['Computador Desktop','Laptop','Servidor','All-in-One','Tablet']

type Campos = Record<string, string | boolean>

const INICIAL: Campos = {
  tipo_equipo:'', placa_activo:'', cod_barras:'', marca:'', modelo:'', numero_serie:'', numero_interno:'',
  ...
  tecnico_responsable:'', contrato_mantenimiento:'', observaciones:'',
}
...
export default function NuevoEquipoPage() {
  ...
  async function guardar() {
    setError('')
    if (!campos.tipo_equipo || !campos.marca) {
      setError('Tipo de equipo y marca son obligatorios')
      return
    }
    setGuardando(true)
    try {
  ...
  return (
    <div style={{ padding: '32px 28px', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 28 }}>
        <Link href="/sistemas/equipos" style={{ color: 'var(--text2)', textDecoration: 'none', fontSize: 14 }}>
      ...
      {esPC && (
        <Seccion titulo="2. Hardware" color="#1e40af">
          ...
          <div style={grid3}>
            <Campo label="Tarjeta de Video" value={campos.tarjeta_video as string} onChange={v => set('tarjeta_video', v)} placeholder="Ej: NVIDIA GeForce RTX 3060" />
      ...
      {esSoftware && (
        <Seccion titulo="4. Software" color="#4f46e5">
          ...
          <div>
            <label style={labelStyle}>Software Adicional</label>
            <textarea
              value={campos.software_adicional as string}
              onChange={e => set('software_adicional', e.target.value)}
              placeholder="Ej: AutoCAD 2023, Adobe Acrobat..."
              rows={3}
      ...
      <Seccion titulo="5. Ubicación" color="#047857">
        <div style={grid3}>
          ...
          <Campo label="Área / Dpto." value={campos.area_ubicacion as string} onChange={v => set('area_ubicacion', v)} placeholder="Ej: Producción, Contabilidad" />
      ...
      <Seccion titulo="6. Estado y Garantía" color="#b45309">
        <div style={grid3}>
          ...
          <Campo label="Forma de Adquisición" value={campos.forma_adquisicion as string} onChange={v => set('forma_adquisicion', v)} placeholder="Ej: Compra, Donación, Arrendamiento" />
        </div>
        <div style={grid3}>
      ...
      <Seccion titulo="7. Mantenimiento Preventivo" color="#7c3aed">
        <div style={grid3}>
          <Campo label="Tipo de Mantenimiento" value={campos.tipo_mantenimiento as string} onChange={v => set('tipo_mantenimiento', v)} placeholder="Ej: Preventivo, Correctivo" />
          <Campo label="Frecuencia" value={campos.frecuencia_mantenimiento as string} onChange={v => set('frecuencia_mantenimiento', v)} placeholder="Ej: Semestral, Anual" />
        ...
        <div style={grid3}>
          ...
          <Campo label="Contrato de Mantenimiento" value={campos.contrato_mantenimiento as string} onChange={v => set('contrato_mantenimiento', v)} />
        </div>
      </Seccion>
      ...
      <Seccion titulo="8. Observaciones" color="#374151">
        <textarea
          value={campos.observaciones as string}
          onChange={e => set('observaciones', e.target.value)}
          placeholder="Notas adicionales, incidencias importantes, configuraciones especiales..."
          rows={4}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8, paddingBottom: 40 }}>
        <Link href="/sistemas/equipos" style={{ padding: '11px 24px', borderRadius: 8, border: '1px solid var(--border)', background: '#fff', color: 'var(--text)', fontWeight: 600, fontSize: 14, textDecoration: 'none' }}>
          Cancelar
        </Link>
        <button onClick={guardar} disabled={guardando}
...
const grid3: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }
const labelStyle: React.CSSProperties = { display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text2)', marginBottom: 5, textTransform: 'uppercase', letterSpacing: '0.04em' }
const inputStyle: React.CSSProperties = { width: '100%', padding: '9px 12px', borderRadius: 8, border: '1px solid var(--border)', fontSize: 14, color: 'var(--text)', outline: 'none', boxSizing: 'border-box' }
