...
export default function GestionUsuariosPage() {
  ...
  async function crearUsuario() {
    ...
    try {
      ...
      setModalOpen(false)
      setNombre(''); setUsername(''); setRol('usuario')
      cargarUsuarios()
    } finally {
      setGuardando(false)
    }
    ...
    <div style={{ padding: '32px 28px', background: '#f8fafc', minHeight: '100vh' }}>
      ...
      {exito && (
        <div style={{
          ...
          padding: '14px 18px', marginBottom: 20, color: '#065f46', fontWeight: 600, fontSize: 14
        }}>
          {exito}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            <thead>
              <tr style={{ background: '#f1f5f9', borderBottom: '2px solid #e2e8f0' }}>
                {['Nombre', 'Usuario', 'Rol', 'Estado', 'Contraseña', 'Creado', 'Acciones'].map(h => (
                  <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, color: '#334155', fontSize: 13, whiteSpace: 'nowrap' }}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                <tr key={u.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{
                      ...
                      color: u.rol === 'admin' ? '#1d4ed8' : '#374151',
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {u.debe_cambiar_password ? (
                      <span style={{ fontSize: 12, color: '#d97706', fontWeight: 600 }}>Pendiente cambio</span>
                    ) : (
                    ...
                    {String(u.id) !== session?.user?.id && (
                      <button
                        ...
                        style={{
                          ...
                          background: u.activo ? '#fff5f5' : '#f0fdf4',
      ...
      {modalOpen && (
        <div onClick={() => setModalOpen(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a', marginTop: 0, marginBottom: 4 }}>
              Nuevo Usuario
            </h2>
            <p style={{ fontSize: 13, color: '#64748b', marginBottom: 24 }}>
              La contraseña inicial será <strong>123456</strong>. El usuario deberá cambiarla en su primer ingreso.
            </p>
